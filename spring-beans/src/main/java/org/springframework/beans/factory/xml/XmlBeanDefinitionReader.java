...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		// getEntityResolver采用什么解析器来解析Stream流
		// getValidationModeForResource 对Resource对象按照什么文件格式来解析发 xsd 还是dtd
	...
	protected int getValidationModeForResource(Resource resource) {
		// 看看当前Resource是什么类型Resource
		int validationModeToUse = getValidationMode();
		if (validationModeToUse != VALIDATION_AUTO) { // 1 表示自动解析
		...
		return VALIDATION_XSD; // 否则就xsd文件解析方式
	}
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		// 创建一个的BeanDefinitionDocumentReader 用来将document对象转换成BeanDefinition对象
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		int countBefore = getRegistry().getBeanDefinitionCount();
		// 转换，创建一个上下文对象 注意这里获取的 META-INF/spring.handlers中的所有类型的处理器
